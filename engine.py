...
from openmdao.main.datatypes.api import Float, Int, Bool, Array
import numpy as np
...
class Engine(Component):
    ...
    #inputs
    RPM = Array([], iotype='in', units='rpm',  desc='Engine RPM')
    throttle = Array([], low=0.01, high=1.0, iotype='in', 
                     desc='Throttle position (from low idle to wide open)')

    power = Array([], iotype='out', units='kW', 
                       desc='Power at engine output')
    torque = Array([], iotype='out', units='N*m', 
                        desc='Torque at engine output')
    fuel_burn = Array([], iotype='out', units='L/s', 
                           desc='Fuel Burn Rate')
    engine_weight = Array([], iotype='out', units='kg', 
                               desc='Engine weight estimation')
    overspeed = Array([], iotype='out', desc='True if the engine RPM '
                            'exceeds its maximum allowable RPM.')
    underspeed = Array([], iotype='out', desc='True if the engine RPM '
    ...
    def __init__(self, h = 4, end_time=764):
        ...
        self.RPM = np.zeros((1,len(self.t))) 
        self.throttle = np.zeros((1,len(self.t)))
        self.power = np.zeros((1,len(self.t)))
        self.torque = np.zeros((1,len(self.t)))
        self.fuel_burn = np.zeros((1,len(self.t)))
        self.overspeed = np.ones((1,len(self.t)), dtype=bool)
        self.underspeed = np.ones((1,len(self.t)), dtype=bool)
